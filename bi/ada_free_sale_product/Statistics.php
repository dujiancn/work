...
class Statistics{
    ...
    private function getProductDetail($productId, $freeSale = false){
        $result = array();
        $sql = "SELECT p.product_id, pd.name as product_name from product as p
    ...
    private function getAllProductId(){
        ...
        $sql = "SELECT product_id FROM product 
                where product_entity_type=0 and active=1 and is_tff=1";
        $queryResult = $this->tffDB->query($sql);
        while($query = mysqli_fetch_assoc($queryResult)){
    ...
    private function getJishiProduct(){
        ...
        $sql = "select obj_id,description from opsp_obj_tags where
                ...
                or ( start_time='0000-00-00' and end_time>'2016-09-30' )
                or ( end_time='0000-00-00' and start_time<'2016-09-29' )
                or ( start_time<'2016-09-29' and end_time>'2016-09-30' ) )";
