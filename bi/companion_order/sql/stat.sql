...
---支付订单详情
select o.order_id,o.created,o.status,ot.value as retail from `order` as o left join order_total as ot on ot.order_id=o.order_id where o.order_id in (select order_id from `order` where created>="2015-01" AND created<"2016-07" and order_id in (select distinct(order_id) from order_status_history where order_id in (select order_id from order_travel_companion) and order_status_id in (100005,100006,100023) ) ) AND ot.class="ot_total";
---执行订单统计
...
---执行订单详情
select o.order_id,o.created,o.status,ot.value as retail from `order` as o left join order_total as ot on ot.order_id=o.order_id where o.order_id in (select order_id from `order` where created>="2015-01" AND created<"2016-07" and order_id in (select distinct(order_id) from order_travel_companion ) and status in (100005,100006)) AND ot.class="ot_total";
